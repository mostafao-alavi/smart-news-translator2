...
export const ReviewStudioTab: React.FC<{ 
  ...
  const fullContent = selectedArticleId ? detailsMap[selectedArticleId]?.content : '';
  const fullTranslatedContent = selectedArticleId ? detailsMap[selectedArticleId]?.translated_content : '';
           ...
           <div className="xl:col-span-3 bg-white border border-gray-200 rounded-2xl overflow-hidden h-[600px] flex flex-col">
              ...
              <div className="overflow-y-auto flex-1 p-2 space-y-2">
                 {news.map(item => (
                    ...
                    >
                       <p className="font-bold text-gray-900 line-clamp-2 leading-relaxed">{item.translated_title}</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-[400px]">
                 ...
                 <div className="bg-gray-50 border border-gray-200 rounded-2xl p-4 flex flex-col">
                    <div className="flex items-center justify-between mb-3 border-b border-gray-200/60 pb-2">
                       ...
                       <a href={selectedArticle?.original_url} target="_blank" rel="noreferrer" className="text-[10px] text-orange-600 hover:underline">View Source</a>
                    </div>
                    <div className="overflow-y-auto flex-1 text-xs text-gray-600 ltr font-mono leading-relaxed space-y-3">
                       <h1 className="font-bold text-gray-900 text-sm">{selectedArticle?.title}</h1>
                 ...
                 <div className="bg-white border border-gray-200 rounded-2xl p-4 flex flex-col shadow-xs">
                    ...
                    <div className="overflow-y-auto flex-1 text-sm text-gray-800 leading-loose text-justify space-y-3">
                       <input 
                         type="text" 
                         defaultValue={selectedArticle?.translated_title || ''} 
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                 <div className="md:col-span-2 bg-gradient-to-br from-indigo-50 to-blue-50 border border-indigo-100 rounded-2xl p-4 shadow-xs">
                    ...
                    <div className="space-y-3">
                       <div>
                          <span className="text-[10px] text-indigo-500 font-bold block mb-1">پیشنهاد عنوان جذاب (Clickbait)</span>
                          <div className="flex gap-2">
                             <div className="flex-1 bg-white border border-indigo-100 rounded-lg px-3 py-1.5 text-xs text-gray-700">
                                بررسی کامل آپدیت جدید هوش مصنوعی در سال ۲۰۲۶
                             </div>
                             <button className="bg-indigo-100 text-indigo-700 hover:bg-indigo-200 px-2 rounded-lg text-[10px] font-bold transition-colors">استفاده</button>
                          </div>
                       </div>
                       <div>
                          <span className="text-[10px] text-indigo-500 font-bold block mb-1">کلمات کلیدی پیشنهادی (Tags)</span>
                          <div className="flex gap-1.5 flex-wrap">
                             <span className="bg-white border border-indigo-100 text-indigo-800 text-[10px] px-2 py-0.5 rounded-full">هوش مصنوعی</span>
                             <span className="bg-white border border-indigo-100 text-indigo-800 text-[10px] px-2 py-0.5 rounded-full">تکنولوژی</span>
                             <span className="bg-white border border-indigo-100 text-indigo-800 text-[10px] px-2 py-0.5 rounded-full">آپدیت ۲۰۲۶</span>
                          </div>
                       </div>
                    </div>
                 </div>
                    ...
                    <div>
                       ...
                       <div className="aspect-video bg-gray-100 rounded-xl border border-gray-200 flex items-center justify-center overflow-hidden relative group">
                          ...
                          <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                             <button className="text-[10px] font-bold text-white bg-white/20 hover:bg-white/30 backdrop-blur-md px-3 py-1.5 rounded-lg">تولید تصویر با AI</button>
                          </div>
                       </div>
                    </div>
                    <div className="flex justify-end gap-2 mt-3">
                       <button className="bg-gray-100 text-gray-600 hover:bg-gray-200 px-3 py-2 rounded-xl text-xs font-bold transition-colors">ذخیره پیش‌نویس</button>
                       <button className="bg-emerald-600 text-white hover:bg-emerald-700 px-4 py-2 rounded-xl text-xs font-bold transition-colors flex items-center gap-1.5 shadow-xs">
                          <Send className="w-3.5 h-3.5" /> انتشار در وردپرس
                       </button>
                    </div>
                 </div>
              </div>
